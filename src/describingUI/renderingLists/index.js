...
exports.ReceipeLists = ReceipeLists;
...
function List() {
    ...
    return (<>
            ...
            <h2>Chemists</h2>
            <ul>
                {chemists.map(person => (<li key={person.id}>
                        <p>
                            <b>{person.name}:</b>
                            {' ' + person.profession + ' '}
            ...
            <ul>
                {anotherPersons.map(person => (<li key={person.id}>
                        <p>
                            <b>{person.name}:</b>
                            {' ' + person.profession + ' '}
                            known for {person.accomplishment}
...
function ReceipeLists() {
    ...
    return (<>
            {recipes.map(item => (<>
                    <h1>{item.name}</h1>
                    <ul>
                        {item.ingredients.map(ingradient => (<li key={item.id}>{ingradient}</li>))}
                    </ul>
                </>))}
        </>);
}
